...
class rooms extends Model {
  ...
  async getRooms(user) {
    try{
      let firstFind = await this.schema.find({firstUser:user});
      let secondFind = await this.schema.find({secondUser:user});
      // console.log('firstUser',firstFind ,'secondUser',secondFind);
      // 
      // console.log('firstUser',firstFind ,'secondUser',secondFind,firstFind !== [] && secondFind !== []);
      if(firstFind.length && secondFind.length ){
        // console.log('1');
        return [...firstFind,secondFind];
      }else if (firstFind.length){
        // console.log('2');
        return [...firstFind];
      }else{
        // console.log('3');
