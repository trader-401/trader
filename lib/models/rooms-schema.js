...
rooms.pre('find',function(next){
  try{
    ...
    this.populate( {path: 'firstImage',select: 'userImage'});
    this.populate({path: 'secondImage',select: 'userImage'});
    next();
  }catch(e){
...
rooms.post('find',async function(){
  try{
    ...
    await this.populate('firstImage').execPopulate((err)=>{
      console.log(err);
    });
    await this.populate('secondImage').execPopulate((err)=>{
      console.log(err);
    });
  }catch(e){
    console.log('this error inside find ---------------->',e);
