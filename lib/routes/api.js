...
function putAllHandler(req, res, next) {
  posts.update(req.params.id, req.body).then((data) => res.json(data));
  // .catch(err=>next(err.message));
}

/////// GET ALL POST BY ADMIN
function getAllHandler(req, res, next) {
  posts.get().then((data) => res.json({ count: data.length, results: data }));
  // .catch((err) => next(err.message));
}

/////// GET ONE POST BY ADMIN
function getOneHandler(req, res, next) {
  posts.get(req.params.id).then((data) => res.json(data));
  // .catch((err) => next(err.message));
}
...
async function facebookHandler(req, res) {
  ...
  const response = await fetch(
    `https://graph.facebook.com/v7.0/me?access_token=${accessToken}&method=get&pretty=0&sdk=joey&suppress_http_code=1`,
  );
...
function showPostsByCategory(req, res, next) {
  ...
  posts.searchGet({ categories: query }).then((data) => {
    console.log(data);
    res.json(data);
  });
  // .catch(err => next(err.message));
