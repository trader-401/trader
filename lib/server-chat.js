...
io.on('connection', socket => {
  ...
  socket.on('joinRoom', async ({ token ,secondUser }) => {
    ...
    socket.emit('message', formatMessage(botName, 'Welcome to trader!'));
    // get msg from db 
    ...
    socket.broadcast
      ...
      .emit(
        'message',
        formatMessage(botName, `${username} is online`),
      );
  ...
  socket.on('disconnect', async() => {
    ...
    if (user1.length) {
      user1.forEach(room => {
        io.to(room._id).emit(
          'message',
          formatMessage(botName, `${username}  is offline right now`),
        );
